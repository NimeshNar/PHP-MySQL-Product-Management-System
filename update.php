...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if (empty($id) || empty($name) || $price === "" || $quantity === "") {
            $error = "All fields are required.";
        } elseif (!filter_var($id, FILTER_VALIDATE_INT) || intval($id) <= 0) {
            ...
            if ($check) {
                ...
                if ($check_res->num_rows === 0) {
                    ...
                    $sql = $conn->prepare("update product set pprice=?,pquantity=?,pname=? where pid=?");
                    if ($sql) {
                        $sql->bind_param('disi', $price_val, $quantity_val, $name, $id_val);
                        if ($sql->execute()) {
...
<html lang="en">
    ...
    <body>
        ...
        <main>
            <h3 class="text-center my-4">Update Product</h3>
            <div class="container text-center update-card my-4 col-md-6">
                <?php if (!empty($error)) { ?>
